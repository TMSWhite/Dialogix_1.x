...
my $triceps_version = '';
my $start_key = 'overall_health_1';
#my $start_key = '.+_severity';
my $stop_key = '.+_imp';
my $start_regex = qr|$start_key|o;
my $stop_regex = qr|$stop_key|o;
my @outlines;
...
sub main {
	...
	print OUT "Filename	Date_Run	Duration	DurSec	StepsTaken	foundStart	foundStop	goodStart	goodStop	calcDur\n";
...
sub processFile {
	...
	my ($timespent,$stimespent,$stepsTaken,$foundStart,$foundStop,$goodStart,$goodStop,$calcdur) = &duration($file);
...
#	print OUT "$title\tv$sched_version\t$schedule\t$filebase\t$when\t$stimespent\t$timespent\t$stepsTaken\n";	
	...
	push @outlines, "$huid\t$when\t$stimespent\t$timespent\t$stepsTaken\t$foundStart\t$foundStop\t$goodStart\t$goodStop\t$calcdur\n";	
...
sub  duration {
	...
	my ($start, $total, $lasttime, $foundStart, $firstRealItemFound, $step, $foundStop, $startcount) = (0, 0, 0, 0, 0, 0, 0, 0, 0);
	my ($goodStart, $goodStop) = (0, 0);
	...
	foreach (@lines) {
		...
		if (/^(.*?)\t(.*?)\t.*?\t.*?\t(\d+)\t/) {
			...
			if (!$firstRealItemFound && $2 =~ /$start_regex/) {
				$firstRealItemFound = 1;	# find the first instance of the start key (so that not replaced if subject backtracks
		...
		if ($foundStart) {
			...
			$foundStart = 0;
			++$startcount;
			last if ($startcount > 1);	# so that doesn't span multiple sessions, which is a problem for CHOICEs, but not others
		}
		if ($foundStop) {
			last;
		}
	...
	my $calcdur = int(($goodStop - $goodStart)/1000);
	$calcdur = -1 if ($goodStart == 0 || $goodStop == 0);
	return ($total,&showDuration($total),$step,$firstRealItemFound,$foundStop,int($goodStart/1000),int($goodStop/1000),$calcdur);
}
